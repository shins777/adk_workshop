...
def get_exchange_rate(
    ...
    """
    두 통화 간의 특정 날짜 환율을 조회합니다.

    Frankfurter API(https://api.frankfurter.app/)를 사용해 환율 데이터를 가져옵니다.

    인자:
        currency_from: 기준 통화(3자리 통화 코드). 기본값은 "USD"(미국 달러)
        currency_to: 대상 통화(3자리 통화 코드). 기본값은 "KRW"(원화)
        currency_date: 환율을 조회할 날짜. 기본값은 "latest"(최신 환율),
            과거 환율은 YYYY-MM-DD 형식으로 지정 가능
